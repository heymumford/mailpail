...
def _get_parser():
    ...
    The parser is expected to be accessible from __main__.py. We import it
    lazily so that test collection doesn't fail if the module hasn't been
    created yet.
    """
    from aol_email_exporter.__main__ import build_parser

    return build_parser()


class TestCLI:
    """Argument parsing for the aol-email-exporter CLI."""
    ...
    def test_parse_minimal_args(self):
        ...
        # Verify defaults
        assert args.format == ["csv"]  # default format
    ...
    def test_parse_all_args(self):
        ...
        args = parser.parse_args([
            ...
            "--sort-by", "date",
            "--reverse",
            "--excel-group-by", "sender",
            "--pdf-title", "My Export",
            "--filename-prefix", "my_export",
            "--log-level", "DEBUG",
        ...
        assert args.excel_group_by == "sender"
        assert args.pdf_title == "My Export"
        assert args.filename_prefix == "my_export"
